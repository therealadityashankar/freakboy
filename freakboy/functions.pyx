...
def data_for_freq(
    ...
    cdef bint frequency_callable, amplitude_callable
    frequency_callable = callable(frequency)
    amplitude_callable = callable(amplitude)

    frequencies = np.empty(frame_count)
    amplitudes = np.empty(frame_count)

    cdef float position_in_frames
    cdef long i

    if frequency_callable:
        for i in range(frame_count):
            position_in_frames = i/frame_count
            frequencies[i] = frequency(position_in_frames)
    else:
        for i in range(frame_count):
            position_in_frames = i/frame_count
    ...
    if amplitude_callable:
        for i in range(frame_count):
            position_in_frames = i/frame_count
            amplitudes[i] = amplitude(position_in_frames)
    else:
        for i in range(frame_count):
            position_in_frames = i/frame_count
            amplitudes[i] = amplitude
    
    wave_values = np.empty(frame_count)

    if wave_type == "sine":
        for i in range(frame_count):
            frames_per_wave = sampling_rate / frequencies[i]
            position_in_wave = i / frames_per_wave
            disp = msin(position_in_wave)
            wave_values[i] = disp
        ...
        for i in range(frame_count):
            frames_per_wave = sampling_rate / frequencies[i]
            position_in_wave = i / frames_per_wave
            disp = square(position_in_wave)
            wave_values[i] = disp

    else:
        raise Error("invalid wave type :" + wave_type)
    
    wavedata = np.empty(frame_count)
    for i in range(frame_count):
        displacement = wave_values[i]
        displacement = displacement * amplitudes[i]
        displacement = displacement * 32767
        displacement = int(displacement)

        for _ in range(number_of_channels):
            wavedata[i] = displacement
...
def msin(float x, bint half=False):
    """
